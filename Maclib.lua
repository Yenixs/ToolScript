...
local function GetGui()
    ...
    if parent then 
        if parent:FindFirstChild("MacLibGui") then
            parent:FindFirstChild("MacLibGui"):Destroy()
        end
    end
	...
	function WindowFunctions:TabGroup()
		...
		function SectionFunctions:Tab(Settings)
			...
			if Settings.Image then
				...
				tabImage.Image = gl(tostring(Settings.Image)).Image
                tabImage.ImageRectSize = gl(tostring(Settings.Image)).ImageRectSize
                tabImage.ImageRectOffset = gl(tostring(Settings.Image)).ImageRectPosition
			...
			function TabFunctions:Section(Settings)
				...
				function SectionFunctions:Slider(Settings, Flag)
					...
					task.spawn(function()
						local v = SliderFunctions.Settings.Default
						SliderFunctions.Value = v
						finalValue = v

						if SliderFunctions.Settings.Callback then
							SliderFunctions.Settings.Callback(v)
						end

						if SliderFunctions.Settings.onInputComplete then
							SliderFunctions.Settings.onInputComplete(v)
						end
					end)
				...
				function SectionFunctions:Input(Settings, Flag)
					...
					task.spawn(function()
						...
						InputFunctions.Text = v

						if InputFunctions.Settings.Callback then
							InputFunctions.Settings.Callback(v)
						end
					end)
				...
				function SectionFunctions:Dropdown(Settings, Flag)
					...
					local function Toggle(optionName, State)
						...
						if State then
							if DropdownFunctions.Settings.Multi then
								...
								Selected = { optionName }
								DropdownFunctions.Value = optionName
							end
					...
					local function addOption(i, v)
						...
						task.spawn(function()
							if not DropdownFunctions.Settings.Callback then return end

							if DropdownFunctions.Settings.Multi then
								DropdownFunctions.Settings.Callback(table.clone(Selected))
							else
								DropdownFunctions.Settings.Callback(Selected[Settings.Default])
							end
						end)
